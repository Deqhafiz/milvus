...
// 3. LikePatternMatcher matches Boost behavior
...
TEST(BackwardCompatibilityTest, MatchOriginalBoostBehaviorForUTF8) {
    ...
    std::vector<std::pair<std::string, std::string>> test_cases = {
        ...
        {"a_b",
         "a\xE4\xBD\xA0"
         "b"},  // a你b with 1 underscore
        {"a__b",
         "a\xE4\xBD\xA0"
         "b"},  // a你b with 2 underscores
        {"a___b",
         "a\xE4\xBD\xA0"
         "b"},  // a你b with 3 underscores

        // 4-byte UTF-8: 😀 (U+1F600) = \xF0\x9F\x98\x80
        {"a___b",
         "a\xF0\x9F\x98\x80"
         "b"},  // a😀b with 3 underscores
        {"a____b",
         "a\xF0\x9F\x98\x80"
         "b"},  // a😀b with 4 underscores
    ...
    for (const auto& [like_pattern, test_str] : test_cases) {
        ...
        LikePatternMatcher smart_matcher(like_pattern);
        ...
        EXPECT_EQ(smart_matcher(test_str), boost_result)
            ...
            << "  Test string bytes: " << test_str.size() << "\n"
            << "  Boost (reference): " << boost_result << "\n"
            << "  LikePatternMatcher: " << smart_matcher(test_str);
    }
}
